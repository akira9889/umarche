            ...
            <div class="lg:flex justify-between items-end">
                <div class="lg:flex items-center">
                    <select name="category" id="category" class="mb-2 lg:mb-0 lg:mr-2">
                        <option value="0"
                            @if (\Request::get('category') === '0') selected @endif>全て</option>
                    ...
                    <div class="flex space-x-2 items-center">
                        <div><input name="keyword" class="border border-gray-500 py-2" type="text"
                                placeholder="キーワードを入力" value="{{ \Request::get('keyword') }}"></div>
                        <div><button
                ...
                <div class="flex">
                    <div>
                        ...
                        <option value="{{ \Constant::SORT_ORDER['recommend'] }}"
                            @if (\Request::get('older') === \Constant::SORT_ORDER['older']) selected @endif>古い順
                        </option>
                    </select>
                </div>
                <div>
                                    ...
                                    <div class="border rouded-md p-2 md:p-4">
                                        ...
                                        <div class="mt-4">
                                            <h3 class="text-gray-500 text-xs tracking-widest title-font mb-1">
                                                {{ $product->category }}</h3>
                                            <h2 class="text-gray-900 dark:text-white title-font text-lg font-medium">
                                                {{ $product->name }}
                                            </h2>
                                            <p class="mt-1">{{ number_format($product->price) }}<span
                                                    class="text-sm text-gray-700 dark:text-gray-400">円(税込)</span></p>
                                        </div>
                                    </div>
                                </a>
                            </div>
